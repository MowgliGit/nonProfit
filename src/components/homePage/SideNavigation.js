...
function SideNavigation() {
  ...
  return (
    <header
      className={`mobileNavigation ${isScrolled ? "scrolled" : ""} ${
        isNotHome ? "boxShadow" : ""
      }`}
    >
      {windowWidth < 990 && (
      ...
      <nav
        className={`sideNav ${menuOpen ? "opened" : ""} ${
          isSubmenuOpen ? "scrollable" : "noScroll"
        }`}
      >
        <ul className="sideNavList">
          {menu.map((item, index) => (
            <li className="sideNavItem" key={item.id}>
              <NavLink
                className={({ isActive }) =>
                  isActive ? "sideNavLink active" : "sideNavLink"
                }
                to={item.link}
              >
                {item.title}
              ...
              {item.subItems && openSubmenuIndex === index && (
                <ul
                  className={`sideSubNavList ${
                    openSubmenuIndex === index ? "subListOpen" : "subListClose"
                  }`}
                >
                  {item.subItems.map((subItem) => (
                    <li key={subItem.id} className="sideSubNavItem">
                      <NavLink className="sideSubNavLink" to={subItem.link}>
                        {subItem.title}
                      </NavLink>
                    </li>
                  ))}
